...
namespace player_impl {
  ...
  void Player::discard(const Tile& tile)
  {
    ...
    table->discards.push_back(i);

    if (!called_riichi) {
      last_discard = table->discards.size() - 1;
    }
  }
  ...
  std::vector<bool> Player::safe_tiles() const
  {
    ...
    for (auto pos = last_discard; pos != table->discards.size(); ++pos) {
      ret[table->discards[pos]] = true;
    }
